...
async def test_extract_input_no_human_message(mocker):
    ...
    assert isinstance(updated_state['messages'][0], SystemMessage)
    assert updated_state['messages'][0].content == mocked_prompt
    initial_state_no_human = GraphState(messages=[SystemMessage(content="Some system message")])
...
async def mock_llm_chain_agent(mocker): 
    ...
    mocker.patch('src.agents.food_agent.llm_selector', return_value=mock_llm_selector_instance)
...
async def test_call_agent_cleans_response(mocker, mock_llm_chain_agent):
    ...
    mocker.patch('src.agents.food_agent.update_prompt', return_value=fixed_prompt)
    raw_response_content = "<think>some thoughts here</think>This is the actual response."
...
async def test_summarize_history_creates_summary_final(mock_llm_for_summarize_node): 
    ...
    initial_state = GraphState(messages=original_messages, human_query="Final query not used by summarize_history")
    updated_state = await summarize_history(initial_state) 
    mock_llm_for_summarize_node.ainvoke.assert_called_once()
    history_str = "\n".join([m.content for m in original_messages if hasattr(m, 'content')])
...
@pytest.mark.asyncio
async def test_summarize_history_empty_history_final(mock_llm_for_summarize_node, mocker):
    ...
    with mocker.patch('src.agents.food_agent.summarize_history', side_effect=guarded_summarize_for_test) as mock_guarded_summarize:
...
async def test_send_response_node_refines_output(mocker, mock_llm_for_send_response):
    mock_update_prompt_call = mocker.patch('src.agents.food_agent.update_prompt', return_value="Test System Prompt Content")
    mock_llm_selector_call = mocker.patch('src.agents.food_agent.llm_selector', return_value=mock_llm_for_send_response)
...
async def test_send_response_node_handles_error(mocker, mock_llm_for_send_response):
    ...
    mock_logger_send_response.assert_called_once_with("Failed to generate final response in send_response.") 
...
def mock_update_prompt_workflow(mocker):
    return mocker.patch('src.agents.food_agent.update_prompt', return_value="System Prompt from Workflow Mock")
...
@pytest.mark.asyncio
async def test_workflow_empty_input_sends_warning(mock_update_prompt_workflow, mock_agent_llm_workflow):
...
@pytest.mark.asyncio
async def test_workflow_normal_flow_calls_agent_once(mock_update_prompt_workflow, mock_agent_llm_workflow, mocker):
    ...
    final_response_llm_ainvoke = MagicMock(return_value=AIMessage(content="Final polished joke"))
    ...
    def llm_selector_side_effect(*args, **kwargs):
        if kwargs.get("openai_model") == "gpt-4o": 
            ...
            mock_llm_selector_instance_for_agent.bind_tools.return_value = mock_bound_tools_for_agent
            return mock_llm_selector_instance_for_agent
    mocker.patch('src.agents.food_agent.llm_selector', side_effect=llm_selector_side_effect)
...
async def test_workflow_tool_usage_recalls_agent(mock_update_prompt_workflow, mocker):
    ...
    mock_agent_llm_ainvoke = MagicMock(
        side_effect=[
            AIMessage(content="", tool_calls=[ToolCall(name="dummy_tool", args={"query": "test"}, id="tc123")]),
            AIMessage(content=agent_response_after_tool)
        ]
    )
    final_response_llm_ainvoke = MagicMock(return_value=AIMessage(content=final_refined_response))
    mock_final_llm = MagicMock()
    mock_final_llm.ainvoke = final_response_llm_ainvoke
    ...
    def llm_selector_side_effect_for_tool_flow(*args, **kwargs):
        if kwargs.get("openai_model") == "gpt-4o": 
            ...
            mock_llm_selector_instance_for_agent.bind_tools.return_value = mock_bound_tools_for_agent
            return mock_llm_selector_instance_for_agent
    mocker.patch('src.agents.food_agent.llm_selector', side_effect=llm_selector_side_effect_for_tool_flow)
...
async def test_workflow_history_summarization_flow(mocker):
    mock_update_prompt_fn = mocker.patch('src.agents.food_agent.update_prompt', return_value="System Prompt from Workflow Mock")
    ...
    summarizer_llm_ainvoke = MagicMock(return_value=AIMessage(content=summarized_history_content))
    agent_llm_ainvoke = MagicMock(return_value=AIMessage(content=agent_response_content))
    final_llm_ainvoke = MagicMock(return_value=AIMessage(content=final_refined_response_content))
    ...
    mocker.patch('src.agents.food_agent.llm_selector', side_effect=[
    ...
    test_workflow = setup_workflow().compile()
    initial_messages = [
        HumanMessage(content="Q1", id="h1"), AIMessage(content="A1", id="a1"),
    ...
    assert final_state['error'] is False
    assert final_state['messages'] == [AIMessage(content=final_refined_response_content)]
    assert final_state['interim_response'] == agent_response_content 
    summarization_history_messages = initial_messages[:-1]
    history_str = "\n".join([m.content for m in summarization_history_messages if hasattr(m, 'content')])
    expected_summarizer_prompt = f"Summarize the following conversation:\n\n{history_str}"
...
async def test_workflow_tool_error_handling(mock_update_prompt_workflow, mocker):
    ...
    agent_llm_ainvoke = MagicMock(
        side_effect=[
            AIMessage(content="", tool_calls=[ToolCall(name="faulty_tool", args={"query": "trigger error"}, id="faulty_tool_call_123")]),
            AIMessage(content=agent_response_after_tool_error) 
        ]
    )
    final_response_llm_ainvoke = MagicMock(return_value=AIMessage(content=final_refined_response_after_error))
    mock_final_llm = MagicMock()
    mock_final_llm.ainvoke = final_response_llm_ainvoke
    ...
    def llm_selector_side_effect_for_tool_error(*args, **kwargs):
        if kwargs.get("openai_model") == "gpt-4o": 
            ...
            mock_llm_selector_instance_for_agent.bind_tools.return_value = mock_bound_tools_for_agent
            return mock_llm_selector_instance_for_agent
    mocker.patch('src.agents.food_agent.llm_selector', side_effect=llm_selector_side_effect_for_tool_error)
...
async def test_workflow_tool_loop_with_history(mocker):
    ...
    H1 = HumanMessage(content="What was the weather yesterday?", id="h1_prev_turn") # Different content
    A1_tool_call = AIMessage(
        content="Checking weather tool for yesterday...", 
        tool_calls=[ToolCall(name="weather_tool_for_loop_test", args={"date": "yesterday"}, id="tc1_prev_turn")], 
        id="a1_tc_prev_turn"
    )
    TM1_tool_response = ToolMessage(content="The weather yesterday was sunny.", tool_call_id="tc1_prev_turn", id="tm1_prev_turn")
    A2_final_response_T1 = AIMessage(content="Yesterday was sunny.", id="a2_final_prev_turn")
    ...
    mock_update_prompt_fn = mocker.patch('src.agents.food_agent.update_prompt', return_value="System Prompt for Loop Test")
    mock_logger_warning = mocker.patch('src.agents.food_agent.logger.warning') # Mock logger.warning
    ...
    def agent_llm_side_effect_for_loop_break_test(*args, **kwargs):
        ...
        return AIMessage(
            ...
            tool_calls=[ToolCall(name="weather_tool_for_loop_test", args={"date": "today"}, id=tool_call_id)]
        )
    ...
    final_response_after_loop_break = "I tried a couple of times but couldn't get the weather for today."
    final_response_llm_ainvoke = MagicMock(return_value=AIMessage(content=final_response_after_loop_break))
    mock_final_llm = MagicMock()
    mock_final_llm.ainvoke = final_response_llm_ainvoke
    ...
    def llm_selector_side_effect_for_loop_break_test_main(*args, **kwargs):
        if kwargs.get("openai_model") == "gpt-4o": # For send_response
            ...
            mock_llm_instance_for_agent.bind_tools.return_value = mock_bound_tools_for_agent
            return mock_llm_instance_for_agent
            
    mocker.patch('src.agents.food_agent.llm_selector', side_effect=llm_selector_side_effect_for_loop_break_test_main)

    # 3. Test Execution
    test_workflow = setup_workflow().compile()
    
    H2_new_query = HumanMessage(content="And today?", id="h2_new_loop_break") # New query
    ...
    warning_message = mock_logger_warning.call_args[0][0] # Get the first positional argument of the call
    ...
    tool_signature_expected = f"weather_tool_for_loop_test|{json.dumps({'date': 'today'}, sort_keys=True)}"
    ...
    assert not final_state['messages'][0].tool_calls # No tool calls in final refined message
    ...
    # +1 by send_response
    # Total = 5
    assert mock_update_prompt_fn.call_count == 1 + mock_call_agent_llm_ainvoke.call_count + 1
    ...
    expected_tracker_key = f"weather_tool_for_loop_test|{json.dumps({'date': 'today'}, sort_keys=True)}"
    assert final_state['tool_call_loop_tracker'] == {expected_tracker_key: 3} # Attempted 3 times
